let inquirer,CHECK_USER,codigaApiFetch,ACTION_TOKEN_ADD,printSuggestion,printFailure,printInfo,printSuccess,setToken,getToken,deleteToken;_3a3‍.x([["checkCodigaToken",()=>checkCodigaToken],["addCodigaToken",()=>addCodigaToken],["deleteCodigaToken",()=>deleteCodigaToken]]);_3a3‍.w("inquirer",[["default",["inquirer"],function(v){inquirer=v}]]);_3a3‍.w("../graphql/queries",[["CHECK_USER",["CHECK_USER"],function(v){CHECK_USER=v}]]);_3a3‍.w("../utils/api",[["codigaApiFetch",["codigaApiFetch"],function(v){codigaApiFetch=v}]]);_3a3‍.w("../utils/constants",[["ACTION_TOKEN_ADD",["ACTION_TOKEN_ADD"],function(v){ACTION_TOKEN_ADD=v}]]);_3a3‍.w("../utils/print",[["printSuggestion",["printSuggestion"],function(v){printSuggestion=v}],["printFailure",["printFailure"],function(v){printFailure=v}],["printInfo",["printInfo"],function(v){printInfo=v}],["printSuccess",["printSuccess"],function(v){printSuccess=v}]]);_3a3‍.w("../utils/store",[["setToken",["setToken"],function(v){setToken=v}],["getToken",["getToken"],function(v){getToken=v}],["deleteToken",["deleteToken"],function(v){deleteToken=v}]]);











/**
 * Returns whether an API token is valid
 * @param {string} apiToken the token string to check
 * @returns {Promise<boolean>} whether the token is valid
 */
async function checkIfTokenIsValid(apiToken) {
  try {
    const resp = await codigaApiFetch(CHECK_USER, null, apiToken);
    if (resp?.user?.id) {
      return true;
    } else {
      return false;
    }
  } catch (err) {
    return false;
  }
}

/**
 * Handles the checking and messaging when checking a
 * token with the `token-check` command action
 */
       async function checkCodigaToken() {
  const token = getToken();
  if (token) {
    if (await checkIfTokenIsValid(token)) {
      printSuccess("A valid API token was found.");
      printSuggestion(
        " ↳ If you wish to override it, run the following command:",
        `codiga ${ACTION_TOKEN_ADD}`
      );
      process.exit(0);
    } else {
      printInfo("An invalid token was found");
      printSuggestion(
        " ↳ To override it, run the following command:",
        `codiga ${ACTION_TOKEN_ADD}`
      );
      process.exit(0);
    }
  } else {
    printInfo("No token was found.");
    printSuggestion(
      " ↳ To set an API token, run the following command:",
      `codiga ${ACTION_TOKEN_ADD}`
    );
    process.exit(1);
  }
}

/**
 * Handles asking, verifying, setting and the messaging
 * when the `token-add` command action is run
 */
       async function addCodigaToken() {
  printSuggestion(
    "Create a Codiga API token:",
    "https://app.codiga.io/api-tokens"
  );
  inquirer
    .prompt([
      {
        type: "input",
        name: "apiToken",
        message: "Please enter your API token:",
      },
    ])
    .then(async ({ apiToken }) => {
      if (await checkIfTokenIsValid(apiToken)) {
        setToken(apiToken);
        printSuccess("Codiga API token added");
        process.exit(0);
      } else {
        printFailure("That token is not valid");
        printSuggestion(
          " ↳ Go to Codiga and create a new token:",
          "https://app.codiga.io/api-tokens"
        );
        process.exit(1);
      }
    })
    .catch((err) => {
      printFailure(err.message);
      process.exit(1);
    });
}

/**
 * Handles deleting a token, if there is one, and the messaging
 * when the `token-delete` command action is run
 */
       async function deleteCodigaToken() {
  if (getToken()) {
    deleteToken();
    // ensure the token was deleted
    if (getToken()) {
      printFailure("We couldn't delete your Codiga API token");
      process.exit(1);
    } else {
      printSuccess("Codiga API token deleted.");
      process.exit(0);
    }
  } else {
    printInfo("No Codiga API token was found to delete.");
    printSuggestion(
      " ↳ To set a Codiga API token, run the following command:",
      `codiga ${ACTION_TOKEN_ADD}`
    );
    process.exit(1);
  }
}
